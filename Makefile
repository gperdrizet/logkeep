...
staging:
	...
	cd $(DOCKER_DIR) && docker-compose -f docker-compose.staging.yml up -d
	@sleep 5
...
staging-logs:
	cd $(DOCKER_DIR) && docker-compose -f docker-compose.staging.yml logs -f

staging-down:
	@echo "🛑 Stopping staging environment..."
	cd $(DOCKER_DIR) && docker-compose -f docker-compose.staging.yml down
...
prod-logs:
	cd $(DOCKER_DIR) && docker-compose -f docker-compose.prod.yml logs -f app-blue app-green
...
clean:
	...
	if [[ $$REPLY =~ ^[Yy]$$ ]]; then \
		...
		cd $(DOCKER_DIR) && docker-compose -f docker-compose.staging.yml down -v; \
		cd $(DOCKER_DIR) && docker-compose -f docker-compose.prod.yml down -v; \
